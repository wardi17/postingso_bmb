  ...
  function setData(result){
    ...
    $.each(result,function(a,b){
      ...
      datatabel +=`
				...
				datatabel +=`<td> <button type="submit" id="gudangpilih" data-sotransacid="${b.SOTransacID}" class="btn  btn-primary">Pilih</button></td>`;
...
function close_attr(nama_file,id){
  ...
  let user ="<?=trim($userlog)?>";
    ...
    $.ajax({
  	...
  	$(document).on("click","#gudangpilih",function(){
		
		 $("#ListGudang").modal("show");
		 let sotransacid = $(this).data('sotransacid');
		  $("#kode_transID").val(sotransacid);
	});
	
	
 $(document).on("click","#save_gudang",function(){
			let kode_transid = $("#kode_transID").val();
		 const gudangID =  $("#gudangID").find(":selected").val();
		
	    $.ajax({
		  url:'<?=base_url?>/listposting/simpangudang',
		  method :'POST',
		  data : {kode_transid:kode_transid,gudangID:gudangID},
		  dataType:'json',
		  ...
		  success:function(result){
			  ...
			  Swal.fire({
					...
					$("#gudangID").prepend("<option value='' selected disabled>Please Select</option>");
					});
...
?>
<!-- Modal -->
<div class="modal fade" id="ListGudang" tabindex="-1" aria-labelledby="ListGudangModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="SendemailModalLabel">Pilih gudang</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
       <input type="hidden" id="kode_transID" class="form-control">
                      <div  class="row col-md-12 mt-4 mb-4">
                                              <label for="gudangID" class="col-md-2 col-form-label">Gudang:</label>
                                              <div class="col-sm-6">
                                              <select class="form-control" id="gudangID">
                                              <option disabled selected="selected">Please Select </option>
                                          <?php  foreach($listgudang as $file => $key):
                                                $kode = $key;
                                                $nama = $file;
      ...
      <div class="text-end">
      <button type="button" class="btn btn-primary" data-bs-dismiss="modal" id="save_gudang">Save</button>
